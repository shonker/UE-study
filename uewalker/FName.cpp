...
auto GetFNameEntryAllocator() -> FNameEntryAllocator* {
	...
	auto offset = 12;
	auto address = find_signature(moduleName, "80 3D ? ? ? ? ? 74 09 48 8D 05 ? ? ? ? EB 13") + offset;
...
auto GetNameDump() -> void {
	...
	for ( short j = 0; j < fNameEntryAllocator->CurrentBlock; ++j ) {
		...
		for (int i = 0; i < FNameEntryAllocator::BlockSizeBytes; ++i) {
			DWORD length = reinterpret_cast<FNameEntryHeader*>(pFNameEntry)->Len >> 5; // It's ok with shifting 5 cuz bIsWide is already cut.
			...
			strncpy_s(tmpName, pFNameEntry + 0x2, length);
